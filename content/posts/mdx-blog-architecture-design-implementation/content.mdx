...
我選擇 MDX 作為部落格架構的核心元素並非沒有救了工作。對於任何建立部落格的開發者來說，這條路充滿了工程挑戰和技術選擇。在選定 MDX 之前，我經歷了一段相當困難的學習曼資。

### 純 TypeScript 方法的失敗經驗

最初，我嘗試使用純 TypeScript 的方法來建立部落格。我的想法是用 TS 創建的應用可以提供全面的型別安全、較高的開發效率和完全性。在 Claude Desktop 和 Windsurf 的幫助下，初期開發非常順利：

1. AI 工具幫助生成基本的介面定義和框架結構
2. 我成功建立了一個有型別安全保障的內容模型
3. 文章的時間戳、標標和湄鍵都工作良好

但是，當我開始對文章加入複雜的 React 組件時，系統很快就變得難以管理：

1. **組件寫入問題**：在純 TypeScript 環境中，將 React 組件嵌入純文本內容需要複雜的解決方案
2. **渲染回湯問題**：在 TS 中文本處理與 JSX 的結合寬易導致各種高沒問題
3. **形式崩壞**：當文章中包含大量代碼範例與嵌入式組件時，整篇文章的格式開始出現各種異常
4. **開發體驗變差**：升級相關包或改變組件介面很容易導致全站出錯

特別是以下具體問題讓我放棄了這種方法：

- 長文章中的代碼高亮導致渲染效能下降
- TypeScript 對 JSX 的型別推斷在複雜文章經常失效
- 大量嵌入式組件导致檔案長度和複雜度爆發
- 寫入文章的流暢性因多重包裝層而大幅下降

最後，文章的格式完全亂掉，我只得放棄這種純 TypeScript 的方法。
