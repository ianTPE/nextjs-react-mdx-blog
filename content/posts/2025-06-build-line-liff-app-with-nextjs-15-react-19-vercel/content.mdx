# 使用 Next.js 15 + React 19 + Vercel 極速打造 LINE LIFF 應用程式
...
### 2. 升級到 Next.js 15 和 React 19

由於 `create-liff-app` 可能還沒有預設使用最新版本，我們需要手動升級：
...
如果遇到相依性警告，可以使用 `--force` 旗標：

```bash
npm install next@latest react@latest react-dom@latest --force
```

### 3. 更新 next.config.js 為 TypeScript

Next.js 15 支援 TypeScript 配置檔，讓我們將 `next.config.js` 改為 `next.config.ts`：
