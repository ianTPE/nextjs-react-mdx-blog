...
直到我用 Claude Opus 4 配合瀏覽器工具和 Playwright 進行深度調試，才終於發現罪魁禍首——**Tailwind CSS Typography 插件**，它的 <code>prose</code> 類在幕後默默地改變著我的表格樣式，而我竟然渾然不覺。
...
一切都從一個看似簡單的需求開始：在一個內容管理系統中展示數據表格。我的 TanStack Table 結構很清晰：
...
我使用了 Tailwind 的標準工具類：<code>w-full</code> 讓表格寬度 100%，<code>px-4 py-2</code> 設定合理的內距。正如您在上面程式碼範例中所見 (第 57 行)，<code>prose</code> 類是套用在包裹整個內容的 <code>&lt;div&gt;</code> 元素上。當時我認為它只影響文字排版，完全沒想到它會對內部的表格「動手腳」。理論上，這一切都應該給我一個乾淨、可控的表格佈局。

### 奇怪的行為

但是當我檢查渲染結果時，發現了令人困惑的現象：

1. **寬度不符預期**：儘管設定了 <code>w-full</code>，表格似乎有自己的想法
2. **高度異常**：表格行的高度比預期的要高，即使我沒有設定任何高度相關的類
3. **內距詭異**：雖然我用了 <code>px-4 py-2</code>，但實際的內距感覺更大
...
**1. 逐行刪除代碼**
我系統性地刪除了每一行 CSS 和 HTML，試圖找到引起問題的確切代碼。但問題是，當我刪除了 <code>prose</code> 類後，雖然表格問題解決了，但整個文章的排版卻亂了——我以為 prose 只是用來美化文字的。
...
最令人沮喪的是瀏覽器 DevTools 的顯示。當我直接檢查表格元素 (<code>&lt;table&gt;</code>) 時，它的 class 列表裡並沒有 <code>prose</code> 這個字眼，這正是我一直強調「表格上根本沒有 prose」的原因，也是困擾我三個月的謎團核心。我期望看到的是我親手寫下的 Tailwind 工具類：
...
問題是，雖然 DevTools 忠實地列出了這些如 <code>.prose table</code>、<code>.prose th</code> 等實際生效的樣式規則，但由於 <code>prose</code> 類本身並不在我正在檢查的 <code>&lt;table&gt;</code> 或 <code>&lt;th&gt;</code> 元素上，而是存在於更上層的父容器，導致我無法直觀地將這些「不速之客」樣式與父層的 <code>prose</code> 類連結起來。它們就這樣混雜在眾多 Tailwind 工具類生成的樣式規則中，像幽靈一樣難以捉摸，極易被忽略。
...
當我運行這個腳本時，我終於看到了完整的樣式繼承鏈。更重要的是，我使用了 Playwright 的 <code>locator.evaluateAll()</code> 方法來檢查所有匹配的 CSS 規則及其來源檔案：
...
**真相大白**：所有這些神秘的樣式都來自 <code>.prose</code> 選擇器！
...
原來，<code>prose</code> 類不僅僅影響文字排版，它還會對**所有子元素**應用預設樣式，包括表格、列表、圖片等等。而 Tailwind Typography 的文檔雖然提到了這一點，但在實際使用中，很容易忽略這個「副作用」。
...
傳統的 Tailwind CSS 會重置所有樣式，所以未經處理的 HTML 內容（例如從 Markdown 文件生成的內容）看起來會非常樸素。Typography 插件通過提供 <code>prose</code> 類來解決這個問題。當您將 <code>prose</code> 類應用到一個父容器元素時，它會自動為該容器內部的所有標準 HTML 子元素——包括段落、標題、列表，以及**本文的主角：表格 (<code>&lt;table&gt;</code>, <code>&lt;th&gt;</code>, <code>&lt;td&gt;</code> 等)**——應用一套精心設計的、美觀的排版樣式。這意味著您無需為每個子元素單獨添加樣式類。

### prose 對表格的具體影響

經過深入研究，我發現 <code>prose</code> 對表格的影響包括：
...
2. **內距被重新定義**，覆蓋了我的 <code>px-4 py-2</code> 設定
3. **邊框自動添加**，即使我沒有設定
4. **margin 自動添加**，影響表格與其他元素的間距

### CSS 特異性的陷阱

更進一步的問題是 CSS 特異性。<code>prose</code> 的選擇器具有更高的特異性：
...
因此，即使我後面添加了 Tailwind 工具類，<code>prose</code> 的樣式仍然會優先生效。
...
1. **樣式混雜與間接影響**：<code>prose</code> 類套用在父容器上，其產生的樣式 (如 <code>.prose table</code>) 雖然會顯示在 DevTools 的計算樣式中，但因為它們並非直接來自被檢查元素 (如 <code>&lt;table&gt;</code>) 自身的 class，而是由父層的 <code>prose</code>「隔空」賦予，這使得它們在眾多 Tailwind 工具類中更難被識別為「元兇」。
...
- 對於表格、列表等重要但非純文字元素的影響，往往只是簡單提及，未能充分警示開發者這種「全局式」套用可能帶來的樣式覆蓋和調試挑戰，尤其是當這些元素本身也有自定義 Tailwind 類時。
- 缺乏實際的調試指南和最佳實踐
...
- [ ] 檢查元素是否在 <code>prose</code> 容器內
- [ ] 使用 DevTools 查看所有應用的 CSS 規則
- [ ] 檢查 CSS 特異性衝突
- [ ] 驗證 HTML 結構和預期是否一致
- [ ] 測試移除 <code>prose</code> 類後的表現
`} />
