...
例如，如果您的 WSL 發行版是 Ubuntu，使用者名為 iantp，則腳本應為：

```bat
@echo off
wsl -d Ubuntu -- bash -ic "cd /mnt/c/Users/iantp/GitHub/mem0/openmemory/api && docker-compose up -d"
```
...
1. 開啟「任務計劃程序」（Task Scheduler），點擊「創建任務」
2. 在「常規」頁簿：
   - 勾選「以最高權限執行」
...
4. 在「操作」頁簿：
   - 新建操作，程序/腳本填寫您剛剛創建的 `.bat` 文件的完整路徑
5. 確定保存

完成上述設置後，每次系統啟動或用戶登入時，OpenMemory 就會自動在後台啟動，無需手動執行命令。

> **專案亮點：**OpenMemory 是一個開源的個人記憶層系統，為大型語言模型（LLMs）提供私有、可攜帶且開源的記憶管理解決方案，讓您完全掌控自己的數據。
